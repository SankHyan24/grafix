...
Intersection BVHAccel::getIntersection(BVHBuildNode *node, const Ray &ray) const
{
    ...
    if (!node->bounds.IntersectP(ray, invdir, dirIsNeg)) // Dont into the box
        return isect;                                    // return null
    if (node->left == nullptr && node->right == nullptr) // Leaf node
        return node->object->getIntersection(ray);       // To the end, get the intersection
    Intersection h1 = getIntersection(node->left, ray);  // Left leaf
    Intersection h2 = getIntersection(node->right, ray); // Right leaf
    return h1.distance < h2.distance ? h1 : h2;          // Return the close one
    return isect;
    // Intersection inter;
